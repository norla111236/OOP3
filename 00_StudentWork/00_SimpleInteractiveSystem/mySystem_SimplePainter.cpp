...
bool SIMPLE_PAINTER::handleKeyPressedEvent( int key )
{
	...
	case(','):
		if(mBrushSize < mMaxBrushSize){
			mBrushSize--;}
		else if (mBrushSize >= mMaxBrushSize){mBrushSize = mMaxBrushSize;}
		break;
	case('.'):
		if(mBrushSize < mMaxBrushSize){
			mBrushSize++;;}
		else if (mBrushSize <= 0){mBrushSize = 0;}
		break;
	}
    return flgHandled;
}
...
void SIMPLE_PAINTER::drawAt(int x, int y, const vector3 &color, double w )
{
	     if(x <= 256 && y<= 256 && x >= 0 && y >= 0){
			  mCanvas[256*x+y] = mCanvas[256*x+y] *(1.0 - (w * w)) + ( w * w * color);}
		 else {
			 mCanvas[256*x+y] = mCanvas[256*x+y];
		 }
